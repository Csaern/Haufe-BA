...
df_queries = pd.read_csv("Data/queries.train.tsv", sep="\t", names=["query_id", "query"], on_bad_lines="skip")
df_qrels = pd.read_csv("Data/qrels.train.tsv", sep="\t", names=["query_id", "doc_id"])
...
num_workers = 64  # Anzahl der parallelen Prozesse
...
def process_query(args):
    ...
    for pos_doc_id in valid_relevant_docs:
        ...
        hard_negatives = get_hard_negatives(valid_relevant_docs, indices, num_negatives=3)
        
        if positive and all(hard_negatives):
            results.append({
                "anchor": query,
                "positive": positive,
                "hard_negative_1": hard_negatives[0],
                "hard_negative_2": hard_negatives[1],
                "hard_negative_3": hard_negatives[2],
            })
...
def create_dataset_parallel(queries_df, qrels_df):
    ...
    for result in results:
        ...
        for entry in result:
            ...
            dataset["hard_negative_1"].append(entry["hard_negative_1"])
            dataset["hard_negative_2"].append(entry["hard_negative_2"])
            dataset["hard_negative_3"].append(entry["hard_negative_3"])
...
dataset.save_to_disk("Mxbai/train_dataset_final_v4")
